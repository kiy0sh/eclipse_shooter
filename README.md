...
PythonでGPhoto2を利用してEOS Rなどのデジタルカメラを操作し、エクセルで指定したスクリプトに沿って自動的に日食の撮影を行います。
Raspberry piなどを利用して稼働させることでPCレスで撮影を行う事が出来ます。

* GPhoto2 対応機種リスト　http://www.gphoto.org/proj/libgphoto2/support.php

* このバージョン(V2)はAEブラケット＋高速連写に対応しており、一部のカメラでは動作しない事が確認されています(EOS 6Dなど)。対応していない場合には、こちらからダウンロードできる旧バージョン(V1)を試してみてください。　https://github.com/kiy0sh/eclipse_shooter/releases/tag/v1.0.0


複数のカメラ接続をサポートしており、起動時に複数のカメラ接続を確認した場合には、どちらのカメラを操作するか選択することが出来ます。
...
サンプルのスクリプト例 (**usa_eclipse.xlsx**) では、
...
* さらに第2接触時間の前後20秒と第3接触時間の前後20秒には1/200～1/50の7段ブラケット撮影
